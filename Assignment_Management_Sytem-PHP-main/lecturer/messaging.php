    ...
    if (empty($message_content)) {
        ...
        try {
            ...
            if (!$insert_stmt) {
                ...
                // Make sure $current_user_id is string
                $current_user_id = (string)$current_user_id;
                
                // Dump the data types and values for debugging
                logError("sender_id: " . gettype($current_user_id) . " = $current_user_id (length: " . strlen($current_user_id) . ")");
                logError("message_content: " . gettype($message_content) . " = $message_content");
                
                // Use the correct bind_param types
                $insert_stmt->bind_param("ss", $current_user_id, $message_content);
