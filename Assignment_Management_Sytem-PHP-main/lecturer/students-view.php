      ...
      <hr>      
        <div class="panel panel-primary filterable" style="border-color: #00bdaa;">
            ...
            <table class="table">
                ...
                <tbody>
                    <?php
                        // Get the logged-in lecturer's user ID
                        // Example: $lecturer_id = $_SESSION['user_id'];
                        // Replace with your actual session variable
                        $lecturer_id = $_SESSION['user_id'];

                        // Get all student IDs assigned to this lecturer
                        $student_ids = [];
                        $sql = "SELECT student_id FROM lecturers WHERE lecturer_id = '$lecturer_id'";
                        $result = mysqli_query($conn, $sql);
                        while ($row = mysqli_fetch_assoc($result)) {
                            $student_ids[] = $row['student_id'];
                        }

                        // If there are assigned students, fetch their details
                        $output = '';
                        if (!empty($student_ids)) {
                                ...
                                $output .= '
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group">
                                                <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">Options <span class="caret"></span></button>
                                                <ul class="dropdown-menu" role="menu">
                                                    <li><a href="add-mark.php?id=' . $username . '">Add Mark</a></li>
                                                    <li><a href="#">Add To Module</a></li>
                                                    <li><a href="#">Add Coursework Mark</a></li>
                                                    <li><a href="#">Add Exam Mark</a></li>
                                                    <li><a href="assign-supervisor.php?id=' . $username . '">Assign Supervisor</a></li>
                                                    <li><a href="edit-student.php?id=' . $username . '">Edit Student Details</a></li>
                                                    <li><a href="remove-student.php?id=' . $username . '">Remove Student</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
                            ...
                            $output = '<tr><td colspan="6">No students assigned.</td></tr>';
                        }
                        echo $output;
                    ?>
                </tbody>
