	...
	if(isset($_POST['submit'])) {
		...
		if($aweight1 <= $remainingWeight) {
			...
			if(isset($_POST['aname']) && isset($_POST['description']) && isset($_POST['nmarks']) && isset($_POST['ascheme'])) {				
				...
				// Send email notification to all students enrolled in this module
				$studentsQuery = "SELECT u.email, u.name, u.surname 
								  FROM users u 
								  JOIN student_modules sm ON u.id = sm.student_id 
								  WHERE sm.module_code = '$mcode' AND u.rank = 'student'";
				
				$studentsResult = mysqli_query($conn, $studentsQuery);
				
				if ($studentsResult && mysqli_num_rows($studentsResult) > 0) {
					// Format deadline date for email
					$deadlineDate = date('l, F j, Y', strtotime($adeadline1));
					...
					try {
						...
						$mail->Password = 'vayn uydp xcjd xupf';            // SMTP password
						$mail->SMTPSecure = 'tls';                            // Enable TLS encryption
						$mail->Port       = 587;                              // TCP port to connect to
						
						// Default sender
						$mail->setFrom('no-reply@university.edu', 'Academic System');
						
						// Set email subject
						$subject = "New Assessment Added: $mcode - $mname";
						...
						while ($student = mysqli_fetch_assoc($studentsResult)) {
							...
							<body>
								...
								<p>A new assessment has been added to your module <strong>$mcode - $mname</strong>.</p>
								
								<h3>Assessment Details:</h3>
								<ul>
									<li><strong>Name:</strong> $aname1</li>
									<li><strong>Weight:</strong> $aweight1%</li>
									<li><strong>Deadline:</strong> $deadlineDate</li>
								</ul>
								
								<p><strong>Description:</strong><br>$adesc</p>
						...
						echo "<div class='alert alert-success' role='alert'>
								<strong>Success!</strong> Assessment has been added and notification emails have been sent to all enrolled students.
							  </div>";
					} catch (Exception $e) {
						// Log the error and display a message
						error_log("Mailer Error: " . $mail->ErrorInfo);
						echo "<div class='alert alert-warning' role='alert'>
								<strong>Note:</strong> Assessment has been added, but there was an issue sending notification emails.
							  </div>";
					}
				}
			}
