    ...
    if($filename != '')
    {
        ...
        if (in_array($ext, $allowed))
        {
            ...
            if (mysqli_query($con, $sql)) {
                ...
                $studentQuery = "SELECT email FROM users WHERE role = 'student'";
                ...
                if ($result && mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        try {
                            ...
                            $mail->SMTPAuth = true;
                            $mail->Username = 'theemaildelivery@gmail.com'; // Replace with your email
                            $mail->Password = 'vayn uydp xcjd xupf'; // Replace with your email password
                            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
                            $mail->Port = 587;

                            $mail->setFrom('theemaildelivery@gmail.com', 'Assignment Management System');
                            $mail->addAddress($studentEmail);

                            $mail->isHTML(true);
                            $mail->Subject = 'New Assignment Assigned';
                            $mail->Body = "Dear Student,<br><br>A new assignment titled <strong>$aname</strong> has been assigned. Please check the system for more details.<br><br>Best regards,<br>Assignment Management System";

                            $mail->send();
                        } catch (Exception $e) {
                            error_log("Email could not be sent to $studentEmail. Error: {$mail->ErrorInfo}");
                        }
                    }
                }
